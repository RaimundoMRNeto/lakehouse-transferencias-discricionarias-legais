"""
Benchmark das consultas Q1 a Q6 no MART FÍSICO (gold.mart_superset_proposta_convenio)
com MÉTRICAS SIMPLIFICADAS (sem COUNT DISTINCT).
"""
import time
...
QUERIES = {
    "Q1 — KPI Propostas (COUNT(*))": (
        "SELECT COUNT(*) FROM gold.mart_superset_proposta_convenio"
    ),
    "Q2 — KPI Convênios (COUNT(numero_convenio))": (
        "SELECT COUNT(numero_convenio) FROM gold.mart_superset_proposta_convenio"
    ),
...
def main():
    print("=" * 80)
    print("BENCHMARK — MART FÍSICO COM MÉTRICAS SIMPLIFICADAS (COUNT SIMPLES)")
    print("=" * 80)
    conn = get_connection()
    results = []
    ...
    print("\n" + "=" * 80)
    print("TABELA RESUMO — MART FÍSICO + MÉTRICAS SIMPLIFICADAS:")
