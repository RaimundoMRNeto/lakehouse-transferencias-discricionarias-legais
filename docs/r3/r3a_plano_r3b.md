...
Camada Silver (Entidades Tipadas e Relacionais)
  ├── silver.siconv_programa [Grão: Programa x Regra de Elegibilidade (1.257.350 linhas)]
  │     └── silver.siconv_programa_cadastral [Grão: 1 linha por ID_PROGRAMA (53.018 linhas) - CONFIRMADO]
  ├── silver.siconv_programa_proposta [Tabela associativa N:N, Grão: (id_programa, id_proposta) (1.158.975 linhas)]
  ├── silver.siconv_proposta [Grão: 1 linha por Proposta, Chave: id_proposta (1.157.619 linhas)]
  └── silver.siconv_convenio [Grão: Instrumento formalizado (287.586 linhas, preservando observações)]
           │
...
1. **Separação de Grãos:** Proposta e Convênio têm grãos de negócio diferentes (1.157.619 propostas vs 287.586 observações de convênio). Apenas 24,8% das propostas se tornam convênio; achatar forçaria 75% da tabela a conter nulos em todas as colunas de convênio.
...
### R3-B4 — Regras de Qualidade e Preservação Controlada
- **Objetivo:** Implementar modelos Silver com padronização de sentinelas (`'-'`), preservação integral das 287.586 observações de convênio e criação das entidades de programa (elegibilidade e cadastral).
- **Arquivos:**
  ...
  - `dbt_lakehouse/models/silver/silver_siconv_programa_cadastral.sql`
- **Dependências:** R3-B3.
- **Riscos:** Descarte indevido de linhas por deduplicação ingênua.
- **Critério de Aceite:** Convênio preserva exatamente 287.586 linhas (sem descartar os 2 conflitos sem critério objetivo de recência); Proposta (1.157.619), Ponte (1.158.975) e Programa Elegibilidade (1.257.350) mantêm 100% das linhas; Programa Cadastral totaliza exatamente 53.018 linhas.
...
- **Dependências:** R3-B4.
- **Riscos:** Falha de testes por pré-condições mal calibradas (ex: testar `unique(numero_convenio)` sabendo da existência legítima de 2 conflitos na fonte).
- **Critério de Aceite:** `dbt test` executado com 100% de testes verdes (`PASS`). O teste `unique` de `numero_convenio` NÃO deve ser ativado.
...
- **Dependências:** R3-B5.
- **Riscos:** Divergência de centavos decorrente de conversão de ponto flutuante (mitigada pelo uso estrito de `DECIMAL(38, 2)`).
- **Critério de Aceite:** Linhas e somas financeiras reconciliadas com tolerância zero (Base `valor_global_convenio` = R$ 356.856.636.504,87).
...
| **Deduplicação arbitrária de convênio** | Alta | Destruição de dados oficiais por presunção de recência inexistente | Preservar as 287.586 observações e documentar os 2 conflitos conhecidos. |
...
| **Data aberrante quebrando agregações de tempo** | Média | Erro em análises cronológicas na Gold | Validação de limites históricos (`BETWEEN '1990-01-01' AND '2050-12-31'`) tratada como REGRA CANDIDATA. |
...
## 5. R3-A.1 — Fechamento dos Findings da Revisão Humana

1. **Grão de `siconv_programa`:** Comprovada a unicidade estrita de 100% dos 1.257.350 registros na chave composta `(ID_PROGRAMA, MODALIDADE_PROGRAMA, NATUREZA_JURIDICA_PROGRAMA, UF_PROGRAMA, ACAO_ORCAMENTARIA)` (zero colisões). Chave primária surrogate padronizada em SHA-256 (MD5 expressamente vedado).
2. **Convênios Conflitantes:** Comprovado que as 2 linhas duplicadas em `949286` e `956078` vieram no mesmo run de ingestão, possuem mesmo SHA-256 de origem, mesmo timestamp técnico e datas oficiais idênticas, divergindo unicamente em `VL_SALDO_CONTA`. Inexistindo critério objetivo de recência, a Silver preservará integralmente as 287.586 linhas, e `unique(numero_convenio)` não é teste obrigatório.
