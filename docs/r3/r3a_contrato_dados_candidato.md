...
- **Volume Esperado:** 287.584 linhas (após deduplicação das 2 linhas de oscilação bancária).
- **Estratégia de Deduplicação:** **DEDUPLICAÇÃO CANDIDATA** recomendada para as 2 linhas anômalas observadas nos convênios `949286` e `956078`.
  - *Critério de Desempate:* `ROW_NUMBER() OVER (PARTITION BY NR_CONVENIO ORDER BY __ingested_at_utc DESC, VL_SALDO_CONTA DESC) = 1`.
  - *Impacto:* Redução de exatamente 2 linhas (de 287.586 para 287.584 linhas), tornando `numero_convenio` 100% único.

| Coluna Bronze | Coluna Silver Candidata | Tipo Bronze | Tipo Silver Candidato | Nullable | Regra de Transformação | Regra de Qualidade | Status |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `NR_CONVENIO` | `numero_convenio` | `string` | `STRING` | NÃO | `TRIM(NR_CONVENIO)` | `not_null`, `unique` | **CONFIRMADA** |
| `ID_PROPOSTA` | `id_proposta` | `string` | `BIGINT` | NÃO | `CAST(ID_PROPOSTA AS BIGINT)` | `not_null`, `unique` | **CONFIRMADA** |
...
| `INSTRUMENTO_ATIVO` | `is_instrumento_ativo` | `string` | `BOOLEAN` | NÃO | `CASE WHEN UPPER(TRIM(INSTRUMENTO_ATIVO)) = 'SIM' THEN TRUE ELSE FALSE END` | `not_null` | **CONFIRMADA** |
| `IND_OPERA_OBTV` | `is_opera_obtv` | `string` | `BOOLEAN` | NÃO | `CASE WHEN UPPER(TRIM(IND_OPERA_OBTV)) = 'SIM' THEN TRUE ELSE FALSE END` | `not_null` | **CONFIRMADA** |
...
| `IND_ASSINADO` | `is_assinado` | `string` | `BOOLEAN` | NÃO | `CASE WHEN UPPER(TRIM(IND_ASSINADO)) = 'SIM' THEN TRUE ELSE FALSE END` | `not_null` | **CONFIRMADA** |
| `MOTIVO_SUSPENSAO` | `motivo_suspensao` | `string` | `STRING` | SIM | `NULLIF(TRIM(MOTIVO_SUSPENSAO), '')` | 93,16% nulos | **CONFIRMADA** |
| `IND_FOTO` | `is_foto` | `string` | `BOOLEAN` | NÃO | `CASE WHEN UPPER(TRIM(IND_FOTO)) = 'SIM' THEN TRUE ELSE FALSE END` | `not_null` | **CONFIRMADA** |
...
### 3.4. Entidade: `silver.siconv_programa` (ou `silver_programa`)
...
- **Chave Primária Lógica Candidata:** Chave composta de elegibilidade ou surrogate key (`id_programa_elegibilidade = MD5(CONCAT(ID_PROGRAMA, '_', MODALIDADE_PROGRAMA, '_', NATUREZA_JURIDICA_PROGRAMA, '_', UF_PROGRAMA, '_', COALESCE(ACAO_ORCAMENTARIA, '')))`.
...
| `DT_PROG_INI_RECEB_PROP` | `data_inicio_recebimento_proposta`| `string`| `DATE` | SIM | `TO_DATE(DT_PROG_INI_RECEB_PROP, 'dd/MM/yyyy')` | Tratar anomalia `0006-03-11` | **CANDIDATA** |
| `DT_PROG_FIM_RECEB_PROP` | `data_fim_recebimento_proposta` | `string` | `DATE` | SIM | `TO_DATE(DT_PROG_FIM_RECEB_PROP, 'dd/MM/yyyy')` | Tratar anomalia `5008-12-15` | **CANDIDATA** |
...
- **`silver.siconv_convenio`**:
  ...
  - `unique`: `numero_convenio` (após deduplicação das 2 linhas), `id_proposta` (após deduplicação).
...
| **Convênio** | 287.586 | **287.584** | Exatamente 2 linhas descartadas por duplicidade de saldo bancário. | 0 variação sobre 287.584 |
| **Programa (Elegibilidade)**| 1.257.350 | **1.257.350** | Zero perdas de critérios de elegibilidade. | 0 linhas |
