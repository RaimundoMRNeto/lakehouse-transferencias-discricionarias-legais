...
| :--- | :--- | :--- | :--- | :--- |
| **Integridade da Fonte** | Hash SHA-256 por arquivo e validação de integridade do ZIP | RAW / Bronze | Script Python de download e extração (`ingest_bronze.py`) | Falha imediata do job; nenhum dado corrompido é gravado no MinIO ou catalogado. |
| **Freshness (Recência)** | Detecção de atualização da fonte via `data_carga_siconv` | Ingestão / RAW | Comparador de metadados na DAG E2E (`dag_lakehouse_e2e`) | Se inalterado: sinaliza `NO_CHANGE` e encerra com sucesso idempotente. Se novo: dispara ciclo completo. |
| **Completude** | Restrição de valores nulos em chaves primárias e atributos essenciais | Silver / Gold / Serving | Teste `not_null` nativo do dbt (`schema.yml`) | Bloqueio imediato da DAG no Airflow; falha do passo dbt test; impede promoção downstream. |
| **Unicidade** | Unicidade estrita de chaves primárias e superchaves de negócio | Silver / Gold / Serving | Teste `unique` do dbt e testes SQL customizados (`tests/*.sql`) | Falha do build dbt; notificação de duplicidade; bloqueio do pipeline analítico. |
...
| **Rastreabilidade** | Propagação de `__ingestion_run_id`, `__source_sha256` e `__source_file` | Todas as camadas | Metadados técnicos herdados de Bronze até Serving | Auditoria forense contínua; qualquer registro analítico pode ser rastreado ao arquivo CSV de origem. |
| **Qualidade Operacional** | Tolerância a falhas, isolamento de tarefas e idempotência | Orquestração (Airflow) | Airflow Fail-Fast e retries configurados nas DAGs | Interrupção atômica; tarefas a jusante não executam sobre estado inconsistente. |
...
6. **Gold Delta**: Modelagem dimensional dimensional em estrela (fatos, dimensões e pontes).
...
sequenceDiagram
    autonumber
    actor User as Operador / Agendamento
    participant E2E as Airflow DAG E2E (R6-B)
    participant ING as Airflow DAG Ingestion (R2)
    participant MINIO as MinIO RAW / Bronze Delta
    participant AUDIT as Quality Gates (Reconciliação)
    participant TRF as Airflow DAG Transformation (R6-A)
    participant DBT as dbt Core + Delta Lake
    participant SUP as Apache Superset

    User->>E2E: Disparo do Pipeline E2E (force_bronze=false/true)
    E2E->>ING: Disparo da DAG de Ingestão
    ING->>ING: Geração do ingestion_run_id (UUID)
    ING->>MINIO: Download ZIP + Cálculo SHA-256
    alt Fonte Não Alterada (NO_CHANGE)
        ING-->>E2E: Status NO_CHANGE (Idempotência)
        E2E-->>User: Pipeline Concluído sem reprocessamento desnecessário
    else Fonte Atualizada ou Carga Forçada
        ING->>MINIO: Carga Bronze Delta + Registro em ingestion_runs e ingestion_manifest
        ING-->>E2E: Ingestão Concluída (Sucesso)
        E2E->>AUDIT: Executa reconcile_bronze_silver.py
        AUDIT-->>E2E: Checkpoint Bronze-Silver PASS
        E2E->>TRF: Disparo da DAG de Transformação com ingestion_run_id
        TRF->>DBT: dbt run --select silver (propaga ingestion_run_id e source_sha256)
        TRF->>DBT: dbt test --select silver
        TRF->>AUDIT: Executa reconcile_silver_gold.py
        AUDIT-->>TRF: Checkpoint Silver-Gold PASS
        TRF->>DBT: dbt run --select gold
        TRF->>DBT: dbt test --select gold
        TRF->>DBT: dbt run --select serving
        TRF->>DBT: dbt test --select serving
        TRF-->>E2E: Transformação e Serving Concluídos com Sucesso
        E2E->>SUP: Serving Mart Atualizado e Pronto para Consumo
        E2E-->>User: Pipeline E2E Aprovado com Sucesso
    end
```
...
- **`ingestion_run_id`**: Identificador UUID único persistido em `bronze.ingestion_runs`.
- **`bronze.ingestion_manifest`**: Metadados de cada arquivo individual (linhas lidas, linhas gravadas, versão Delta).
- **`__source_sha256`**: Assinatura criptográfica que acompanha os dados pelas transformações Silver e Gold.
