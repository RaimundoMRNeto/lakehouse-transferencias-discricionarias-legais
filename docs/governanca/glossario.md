...
- **Serving Mart**: Tabela Delta Lake materializada fisicamente a partir da Semantic View (`mart_superset_proposta_convenio`), oferecendo consultas sub-segundo e eliminando sobrecarga computacional no consumo do Apache Superset.
...
- **ingestion_run_id**: Identificador universalmente único (UUID) gerado no início de cada execução da pipeline de ingestão e propagado por todas as tabelas downstream, assegurando rastreabilidade forense completa.
