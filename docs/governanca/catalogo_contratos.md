...
| **Bronze** | `ingestion_runs` | 1 linha / execução de ingestão | `ingestion_run_id` | R2 / AuditManager | Delta Lake (tabela) | Não aplicável (auditoria) | Governança / Auditoria | UPSERT idempotente por execução; registra estado global e retenção. |
| **Bronze** | `ingestion_manifest` | 1 linha / dataset / execução | (`ingestion_run_id`, `dataset_id`) | R2 / AuditManager | Delta Lake (tabela) | Não aplicável (auditoria) | Governança / Auditoria | Registro idempotente de proveniência, SHA-256, contagens, path e versão Delta. |
